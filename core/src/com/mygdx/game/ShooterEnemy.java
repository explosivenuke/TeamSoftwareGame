...
public class ShooterEnemy extends Entity{
	...
	public ShooterEnemy(float spawnX, float spawnY, float width, float height, float moveSpeed, double maxHealth, double damage,
			boolean inv, Texture img, SpriteBatch map) {
		super(spawnX, spawnY, width, height, moveSpeed, maxHealth, damage, inv, img, map, false);
	}
	...
	public Bullet trackPlayer(Entity player) {
		...
		return Spawner.spawnBullet(getPosX(), getPosY(), sizeX, sizeY, 2, 100, true, batch, false, player.getCenterX(), player.getCenterY());
	}
